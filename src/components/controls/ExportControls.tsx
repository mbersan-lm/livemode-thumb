  ...
  const handleExportAoVivo = async () => {
    ...
    try {
      ...
      const canvas = await html2canvas(canvasRefAoVivo.current, {
        ...
        foreignObjectRendering: false,
        onclone: (clonedDoc) => {
          const canvas = clonedDoc.getElementById('CANVAS_AO_VIVO');
          if (!canvas) return;
          ...
          const homeScore = clonedDoc.getElementById('AV_HOME_SCORE') as HTMLElement;
          const xChar = clonedDoc.getElementById('AV_X_CHAR') as HTMLElement;
          const awayScore = clonedDoc.getElementById('AV_AWAY_SCORE') as HTMLElement;
      ...
      const filename = `AV_${homeTeam?.slug || 'home'}_${awayTeam?.slug || 'away'}_${matchData.homeScore}x${matchData.awayScore}.jpg`;
      ...
      canvas.toBlob((blob) => {
        if (blob) {
          ...
          link.click();
          URL.revokeObjectURL(url);
          toast.success('JPG Ao Vivo exportado!');
        }
      }, 'image/jpeg', 0.9);
    } catch (error) {
      console.error('Export error:', error);
      toast.error('Falha ao exportar JPG Ao Vivo');
    }
  };
